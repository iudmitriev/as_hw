...
class SincConv_fast(nn.Module):
    ...
    def __init__(self, out_channels, kernel_size, sample_rate=16000, in_channels=1,
                 stride=1, padding=0, dilation=1, bias=False, groups=1, min_low_hz=50, min_band_hz=50, inverse_mel=False):
        ...
        if inverse_mel:
            mel = np.linspace(
                1 / self.to_mel(high_hz), 1 / self.to_mel(low_hz), self.out_channels + 1
            )
            mel = np.flip(1 / mel)
        else:
...
class FMS(nn.Module):
    def __init__(self, filter_length):
        super().__init__()
        self.avpool = nn.AdaptiveAvgPool1d(output_size=1)
...
class ResBlock(nn.Module):
    def __init__(self, in_channels: int, out_channels: int):
        ...
        if in_channels != out_channels:
            self.channel_equalizer = nn.Conv1d(
                in_channels=in_channels,
                out_channels=out_channels,
                kernel_size=1,
                padding="same",
        ...
        self.head = nn.Sequential(
            ...
            FMS(
                filter_length=out_channels
            )
        )

    def forward(self, x):
        out = self.body(x)
        if self.channel_equalizer is not None:
            x = self.channel_equalizer(x)
        out = out + x
        out = self.head(out)
        return out
